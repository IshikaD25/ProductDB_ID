...
conn = pyodbc.connect(
    ...
    'SERVER=DESKTOP-AR3SJT4\\SQLEXPRESS;'
    'DATABASE=ProductDB;'
...
def show_menu():
    ...
    print("1️⃣  Insert New Product")
    print("2️⃣  View All Products")
    # print("3️⃣  Update Product")   # temporarily disabled
    # print("4️⃣  Delete Product")   # temporarily disabled
    print("5️⃣  Search Products")
    print("6️⃣  Exit")
...
def delete_product():
    ...
    conn.commit()
    print(f"🗑️ Product ID {pid} deleted successfully!")
"""
...
def search_products():
    print("\n🔍 SEARCH OPTIONS")
    print("1️⃣  Search by Product Name")
    print("2️⃣  Search by Price Range")
    print("3️⃣  Search by Quantity Range")
    ...
    rows = cursor.fetchall()
    if not rows:
        print("⚠️ No products match your search criteria.")
    else:
        print("\n📦 Search Results:")
        for row in rows:
            print(f"ID: {row.ProductID}, Name: {row.ProductName}, Price: {row.Price}, Quantity: {row.Quantity}")
